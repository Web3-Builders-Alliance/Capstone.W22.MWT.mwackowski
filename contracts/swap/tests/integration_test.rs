...
use osmo_swap::msg::{
    ArithmeticTwapToNowResponse, ExecuteMsg, QueryEpochsInfoResponse,
...
fn test_any_balancer_pool_response_deser() {
    with_env_setup(
        |app, wasm, signer, _code_id, contract_addr| {
            ...
            assert_eq!(pool, helpers::mock_balancer_pool());
        },
        false,
    );
}
...
fn test_any_balancer_pool_params_response_deser() {
    with_env_setup(
        |app, wasm, signer, _code_id, contract_addr| {
            ...
            assert_eq!(pool, helpers::mock_balancer_pool().pool_params.unwrap());
        },
        false,
    );
}
            ...
            let swap = || {
                app.execute::<_, MsgSwapExactAmountInResponse>(
                    gamm::v1beta1::MsgSwapExactAmountIn {
                        ...
                        token_in: Some(Coin::new(100, "uosmo").into()),
