...
const swap_code_id = constants.swap_code_id;
const cw20_code_id = constants.cw20_code_id;
const manager_code_id = constants.manager_code_id;

const swap_addr = constants.swap_addr;
const manager_addr = constants.manager_addr;
const cw20_addr1 = constants.cw20_addr1;
const cw20_addr2 = constants.cw20_addr2;
...
describe("swap Fullstack Test", () => {
    ...
    xit("1. Execute first swap", async() => {
        let client = await setupClient(mnemonic, rpcEndpoint, "0.025uosmo");
        let osmo_amnt_before = await client.getBalance(await getAddress(mnemonic), "uosmo");
        let res = await client.execute(await getAddress(mnemonic), 
        ...
        let osmo_amnt_after = await client.getBalance(await getAddress(mnemonic), "uosmo");
        for (let i = 0; i<res.logs[0].events.length; i++) {
            console.log("------------EVENTS[%s]-----------------",i);
            console.log(res.logs[0].events[i]);          
        };
        console.log("Total osmo balance change: %d", Number(osmo_amnt_after['amount'])-Number(osmo_amnt_before['amount']));
    }).timeout(20000);
    ...
    xit("5. Execute swap by a second user", async() => {
        ...
        let osmo_amnt_before = await client.getBalance(await getAddress(mnemonic_second_user), "uosmo");
        let res = await client.execute(await getAddress(mnemonic_second_user), 
            manager_addr, {  swap_tokens: { 
                initial_balance: {amount: "2500000", denom: "uosmo"}, 
                etf_swap_routes: 
                {
                    name: "Bull_Market_Is_Coming",
                    routes: 
            ...
            "auto", "", 
            [{amount: "2500000", denom: "uosmo"}]
        );
        let osmo_amnt_after = await client.getBalance(await getAddress(mnemonic_second_user), "uosmo");
        for (let i = 0; i<res.logs[0].events.length; i++) {
            console.log("------------EVENTS[%s]-----------------",i);
            console.log(res.logs[0].events[i]);          
        };
    ...
    xit("12. Redeem second swap first user", async() => {
        let client = await setupClient(mnemonic, rpcEndpoint, "0.025uosmo");
        let osmo_amnt_before = await client.getBalance(await getAddress(mnemonic), "uosmo");
        let res = await client.execute(await getAddress(mnemonic), 
        manager_addr, {  redeem_tokens: { 
    ...
    xit("13. Redeem first swap second user", async() => {
        let client = await setupClient(mnemonic_second_user, rpcEndpoint, "0.025uosmo");
        let osmo_amnt_before = await client.getBalance(await getAddress(mnemonic_second_user), "uosmo");
        let res = await client.execute(await getAddress(mnemonic_second_user), 
        manager_addr, {  redeem_tokens: { 
